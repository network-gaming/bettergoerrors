...
func TestUnwrapAll(t *testing.T) {
	...
	werr := UnwrapAll(err)
	if werr != rootErr { //nolint:errorlint,goerr113 // We want to compare the current error.
...
func TestUnwrapAllNone(t *testing.T) {
	...
	werr := UnwrapAll(err)
	if werr != err { //nolint:errorlint,goerr113 // We want to compare the current error.
