...
func WithTag(err error, key string, value string) error {
	if err == nil {
		return nil
	}
	return &tag{
...
func (err *tag) WriteErrorMessage(w Writer, verbose bool) bool {
	if !verbose {
		return false
	}
	_, _ = w.WriteString("tag ")
	_, _ = w.WriteString(err.key)
	_, _ = w.WriteString(" = ")
	_, _ = w.WriteString(err.value)
	return true
}
